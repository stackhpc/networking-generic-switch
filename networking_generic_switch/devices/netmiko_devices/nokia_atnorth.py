...
class NokiaSRLAtNorth(netmiko_devices.NetmikoSwitch):
    ADD_NETWORK = (
        'set tunnel-interface vxlan1 vxlan-interface {segmentation_id} '
        'type bridged',
        'set tunnel-interface vxlan1 vxlan-interface {segmentation_id} '
        ...
        'vxlan1.{segmentation_id}',
        'set network-instance mac-vrf-{segmentation_id} protocols bgp-evpn '
        'bgp-instance 1 vxlan-interface vxlan1.{segmentation_id}',
        'set network-instance mac-vrf-{segmentation_id} protocols bgp-evpn '
        'bgp-instance 1 evi {segmentation_id}',
        'set network-instance mac-vrf-{segmentation_id} protocols bgp-evpn '
        'bgp-instance 1 ecmp 2',
        'set network-instance mac-vrf-{segmentation_id} protocols bgp-vpn '
        'bgp-instance 1 route-target export-rt target:64601:{segmentation_id}',
        'set network-instance mac-vrf-{segmentation_id} protocols bgp-vpn '
        'bgp-instance 1 route-target import-rt target:64601:{segmentation_id}',
    )

    DELETE_NETWORK = (
        'delete network-instance mac-vrf-{segmentation_id}',
        'delete tunnel-interface vxlan1 vxlan-interface {segmentation_id}',
    )
